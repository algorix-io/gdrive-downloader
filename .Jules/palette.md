...
## 2026-04-01 - [Synchronous Form Feedback]
**Learning:** Adding immediate visual feedback (like disabling buttons and changing text) to synchronous form submissions prevents duplicate clicks and improves the user's perception of performance, especially on slower connections or with complex server-side processing.
**Action:** Always implement disabled states and "loading" text/spinners on submit buttons, even when the form submission is synchronous. Ensure the feedback matches the application's theme (e.g., "[DECRYPTING...]").
