...
## 2026-05-15 - [Semantic Input Styling Consistency]
**Learning:** Changing input types for better mobile UX (e.g., `text` to `url`) can inadvertently break custom CSS styling if the selectors are type-specific. Semantic enhancements must be paired with CSS updates to preserve the visual design.
**Action:** When introducing new semantic input types (like `type="url"`), always review and update existing CSS selectors (e.g., appending `input[type=url]` to `input[type=text]` rules) to ensure visual consistency.
