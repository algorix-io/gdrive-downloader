...
## 2026-05-18 - [Semantic Input Types for Mobile]
**Learning:** Using `type="text"` for URL or email fields causes friction on mobile devices because standard keyboards appear. Using semantic types like `type="url"` triggers optimized keyboards (e.g., with quick `.com` or `/` keys), significantly speeding up input for mobile users.
**Action:** Always use semantic HTML5 input types (`url`, `email`, `tel`, etc.) instead of generic `text` inputs, and ensure these new selectors are added to existing CSS rules to maintain consistent styling.
