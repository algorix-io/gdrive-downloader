...
## 2026-05-17 - [Semantic Input Types and Mobile UX]
**Learning:** Using semantic input types like `type="url"` instead of `type="text"` provides immediate UX benefits on mobile by triggering the appropriate virtual keyboard, without requiring extra JS validation. However, you must explicitly add these new input types to existing CSS selectors, otherwise they lose their styling and break the visual theme.
**Action:** When swapping generic inputs for semantic ones (url, email, tel), always  for the old input type in the CSS and ensure the new type is added to all relevant rule sets to preserve the design system.

## 2026-05-17 - [Semantic Input Types and Mobile UX]
**Learning:** Using semantic input types like `type="url"` instead of `type="text"` provides immediate UX benefits on mobile by triggering the appropriate virtual keyboard, without requiring extra JS validation. However, you must explicitly add these new input types to existing CSS selectors, otherwise they lose their styling and break the visual theme.
**Action:** When swapping generic inputs for semantic ones (url, email, tel), always grep for the old input type in the CSS and ensure the new type is added to all relevant rule sets to preserve the design system.
