...
def run():
    with sync_playwright() as p:
        ...
        if page.locator("input[name='access_password']").count() > 0:
            print("Verifying login button loading state...")

            # Setup evaluate listener to prevent default so we can check the state
            page.evaluate("""() => {
            ...
            print("Logging in...")
            # Reload page to reset form state and proceed with actual login
            page.reload()
            page.fill("input[name='access_password']", "matrixCore2025")
            page.click("#loginBtn")
